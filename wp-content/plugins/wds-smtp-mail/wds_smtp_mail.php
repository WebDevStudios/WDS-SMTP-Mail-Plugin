...
global $wds_smtp_options;
$wds_smtp_options = array(
	'mail_from' => null,
	'mail_from_name' => null,
	'mailer' => 'smtp',
...
function wds_smtp_activate() {
	global $wds_smtp_options;
		
	foreach( $wds_smtp_options as $name => $value )
		add_option( $name, $value );
}

function wds_smtp_whitelist_options( $whitelist_options ) {
	global $wds_smtp_options;
		
	$whitelist_options['email'] = array_keys( $wds_smtp_options );
	return $whitelist_options;
}

function wds_phpmailer_init_smtp( $phpmailer ) {
	...
	$phpmailer = apply_filters('wds_smtp_custom_options', $phpmailer);
}

function wds_smtp_options_page() {
	global $wds_smtp_options, $phpmailer;
...
function wds_smtp_menus() {
	if( function_exists('add_submenu_page') ) 
	{
		add_options_page(__('Advanced Email Options', 'wds_smtp'),__('Email', 'wds_smtp'),'manage_options',__FILE__,'wds_smtp_options_page');
	}
}

# Sets the from email value
function wds_smtp_mail_from( $orig ) {
 ...
 */
function wds_smtp_mail_from_name( $orig ) {
...
function wds_smtp_plugin_action_links($links, $file) {
...
add_action( 'phpmailer_init','wds_phpmailer_init_smtp' );

# Whitelist our options
add_filter( 'whitelist_options', 'wds_smtp_whitelist_options' );

# Add the create pages options
add_action( 'admin_menu','wds_smtp_menus' );

# Add an activation hook for this plugin
register_activation_hook( __FILE__,'wds_smtp_activate' );

# Adds "Settings" link to the plugin action page
add_filter( 'plugin_action_links', 'wds_smtp_plugin_action_links',10,2 );

# Add filters to replace the mail from name and email address
add_filter( 'wp_mail_from','wds_smtp_mail_from' );
add_filter( 'wp_mail_from_name','wds_smtp_mail_from_name' );